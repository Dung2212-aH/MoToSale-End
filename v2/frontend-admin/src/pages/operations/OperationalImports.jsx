...
const OperationalImports = () => {
  ...
  const parse = async (event) => {
    ...
    try {
      const sheet = await readSheet(file); const headers = headerMap(sheet); const next = [];
      for (let index = 2; index <= sheet.rowCount; index += 1) {
        const row = sheet.getRow(index);
        if (tab === 'products') {
          const item = {
            row: index, code: normalize(cell(row, headers, ['mã sản phẩm', 'ma san pham', 'code'])),
          ...
          const item = { row: index, storeId: number(cell(row, headers, ['mã kho', 'ma kho', 'storeid'])), skuId: number(cell(row, headers, ['mã sku', 'ma sku', 'skuid'])), qty: number(cell(row, headers, ['tồn đầu kỳ', 'ton dau ky', 'quantity', 'qty'])) };
          if (!item.storeId && !item.skuId && !item.qty) continue;
          item.errors = [!item.storeId && 'Thiếu mã kho', !item.skuId && 'Thiếu mã SKU', !(item.qty >= 0) && 'Tồn đầu kỳ không hợp lệ'].filter(Boolean);
          next.push(item);
        }
      }
      setRows(next);
    } catch (err) { alert(`Không thể đọc XLSX: ${err.message}`); }
  };
  ...
  const commit = async () => {
    ...
    setSaving(true); const errors = [];
    try {
      if (tab === 'products') {
        for (const row of rows) {
          try {
            const created = await productService.create({ code: row.code, name: row.name, categoryId: row.categoryId, brandId: row.brandId, kind: row.kind, listPrice: row.listPrice, salePrice: row.salePrice, shortDescription: 'Import XLSX' });
            await productService.createVariant(created.data.id, { skuCode: row.skuCode, variantName: row.variantName || 'Mặc định', listPrice: row.listPrice, salePrice: row.salePrice });
          } catch (err) { errors.push(`Dòng ${row.row}: ${err?.response?.data?.message || 'không thể lưu'}`); }
        }
      } else {
        const lookups = (await businessOperationsService.getLookups()).data;
        for (const row of rows) {
          try {
            if (!lookups.stores.some((x) => x.id === row.storeId) || !lookups.skus.some((x) => x.id === row.skuId)) throw new Error('Mã kho hoặc SKU không tồn tại');
            await inventoryService.adjustStock({ storeId: row.storeId, skuId: row.skuId, transactionType: 'Adjust', qty: row.qty, reason: 'Import tồn đầu kỳ từ XLSX' });
          } catch (err) { errors.push(`Dòng ${row.row}: ${err?.response?.data?.message || err.message || 'không thể lưu'}`); }
        }
      }

...
const BusinessOperations = ({ section = 'supply' }) => {
  ...
  return (
    <div className="content-wrapper">
      ...
      {paymentTarget && <Modal title={`Thanh toán đơn mua ${paymentTarget.code}`} close={() => setPaymentTarget(null)} save={payPurchase}><Input label="Số tiền" type="number" value={payment.amount} set={(x) => setPayment({ ...payment, amount: x })} /><Select label="Hình thức" value={payment.method} set={(x) => setPayment({ ...payment, method: x })} items={[{ id: 'Cash', name: 'Tiền mặt' }, { id: 'BankTransfer', name: 'Chuyển khoản' }]} text={(x) => x.name} /><Input label="Ghi chú" value={payment.note} set={(x) => setPayment({ ...payment, note: x })} /></Modal>}

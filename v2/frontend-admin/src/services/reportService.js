...
const getOrderAmount = (order) => Number(order?.tongThanhToan ?? order?.tongTien ?? order?.totalAmount ?? order?.grandTotal ?? order?.amount ?? 0);

const getOrderStatus = (order) => order?.trangThaiDonHang || order?.TrangThaiDonHang || order?.trangThai || order?.status || order?.orderStatus || '';
...
const getDateValue = (item) => item?.ngayTao || item?.NgayTao || item?.createdAt || item?.createdDate || item?.placedAt || item?.paidAt || item?.ngayThanhToan;

const getPaymentDateValue = (item) => item?.ngayThanhToanThanhCong || item?.NgayThanhToanThanhCong || item?.ngayThanhToan || item?.paidAt || item?.placedAt || item?.createdAt || item?.ngayTao;
...
const getOrderItems = (order) => order?.items || order?.Items || order?.chiTiet || order?.ChiTiet || order?.lines || [];
...
const buildTopProductsFromOrders = (orders, limit) => {
  ...
  orders.filter(isRevenueOrder).forEach((order) => {
    getOrderItems(order).forEach((item) => {
      ...
      const quantity = Number(item?.soLuong ?? item?.SoLuong ?? item?.quantity ?? item?.qty ?? 0);
      const revenue = Number(item?.thanhTien ?? item?.ThanhTien ?? item?.lineTotal ?? 0) ||
...
const reportService = {
  getSummary: async () => {
    try {
      ...
      return normalizeRemoteReport(response);
    } catch {
      // Fallback for older backend builds while v2 services are being migrated.
    }
  ...
  getReports: async ({ startDate, endDate }) => {
    try {
      const response = await api.get('/reports', { params: { startDate, endDate, top: 10 } });
      return normalizeRemoteReport(response);
    } catch {
      // Fallback for older backend builds while v2 services are being migrated.
    }

...
public static class SeedConfiguration
{
    ...
    private static async Task AddMissingOperationalDataAsync(AppDbContext db, DateTime now)
    {
        ...
        // Đơn hàng mẫu gắn với tài khoản demo. Không seed tài khoản tự động nữa nên nếu các
        // tài khoản này không tồn tại thì bỏ qua dữ liệu vận hành mẫu (tránh KeyNotFoundException).
        string[] requiredEmails =
        [
            "staff@motosale.local", "customer@motosale.local", "minhanh@example.com",
            "quochuy@example.com", "hoangnam@example.com", "thutrang@example.com",
        ];
        if (requiredEmails.Any(email => !users.ContainsKey(email))) return;

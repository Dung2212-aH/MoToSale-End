...
public record CreatePurchaseOrderRequest(int SupplierId, int StoreId, string? Note, List<PurchaseLineRequest> Lines);
...
public record CreateRepairOrderRequest(int CustomerId, int StoreId, int? AssignedStaffId, int? WarrantyId, string VehicleDescription, string ReportedIssue, decimal LaborCost, string? Note, List<RepairLineRequest>? Lines);
...
public record AttendanceRequest(int StaffUserId, int StoreId, string? Note);

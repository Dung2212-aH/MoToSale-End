...
{
    // Bí danh tương thích frontend cũ: SPA đọc image.imageUrl (camelCase) trước tiên.
    public string ImageUrl => Url;
}

// Biến thể theo hình dạng frontend cũ mong đợi. id = Sku.Id (frontend gửi lại làm maBienSanPham khi thêm giỏ hàng).
public record ProductVariantDto(
    int Id, string Sku, string? VariantName, decimal PriceOverride, int StockQuantity,
    string? Color, string? Version, int Status);
...
public record ProductListItem(
    ...
    // ===== Lớp tương thích frontend cũ (additive, camelCase) =====
    string ProductCode, string? CategoryName, string? BrandName, int? CarModelId, string? CarModelName,
    int ProductType, decimal BasePrice, int StockQuantity, double AverageRating, int TotalReviews,
    IEnumerable<ProductVariantDto> Variants, IEnumerable<ProductImageDto> Images);
...
public record ProductDetail(
    ...
    // ===== Lớp tương thích frontend cũ (additive, camelCase) =====
    string ProductCode, string? CategoryName, string? BrandName, int? CarModelId, string? CarModelName,
    int ProductType, decimal BasePrice, decimal? SalePrice, int StockQuantity,
    string? MainImageUrl, double AverageRating, int TotalReviews,

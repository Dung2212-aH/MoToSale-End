...
public class CatalogService : ICatalogService
{
    ...
    public async Task<PagingResponse<ProductListItem>> SearchProductsAsync(ProductSearchRequest request)
    {
        ...
        var names = await NameMapsAsync(page.Items);
        var reviews = await ReviewAggAsync(page.Items.Select(p => p.Id));
        return new PagingResponse<ProductListItem>
        {
            Items = page.Items.Select(p => MapListItem(
                ...
                names, reviews.GetValueOrDefault(p.Id), onHand)).ToList(),
            Page = page.Page,
    ...
    public async Task<ProductDetail?> GetProductAsync(int id)
    {
        ...
        var names = await NameMapsAsync(new[] { p });
        var reviews = (await ReviewAggAsync(new[] { p.Id })).GetValueOrDefault(p.Id);
        var onHand = await _inventory.GetOnHandBySkusAsync(p.Skus.Select(s => s.Id));
        return MapDetail(p, mfgName, names, reviews, onHand);
    }
    ...
    {
        // Join Favorite -> Product, kèm SKU (giá) và ảnh (ảnh chính). Mới nhất lên đầu.
        ...
        return favorites
            .Where(f => f.Product is not null)
            .Select(f => new FavoriteDto(f.Id, f.UserId, f.ProductId, f.CreatedDate, MapFavoriteProduct(f.Product!)))
            .ToList();
    }
    ...
    public async Task AddFavoriteAsync(int userId, int productId)
    {
        ...
        // Idempotent: nếu đã yêu thích thì bỏ qua.
        if (await _favorites.AnyAsync(f => f.UserId == userId && f.ProductId == productId))
            return;
    ...
    {
        // Idempotent: không có thì coi như đã xóa.
        var existing = (await _favorites.FindAsync(f => f.UserId == userId && f.ProductId == productId)).FirstOrDefault();
        if (existing is null) return;
        _favorites.Delete(existing);
        await _favorites.SaveChangesAsync();
    }

    private static FavoriteProductDto MapFavoriteProduct(Product p)
    {
        ...
        return new FavoriteProductDto(
            ...
            sku?.ListPrice ?? 0, sku?.SalePrice, image?.Url, p.Status);
    }

    // ===== Lookup phục vụ lớp tương thích frontend cũ =====
    private async Task<NameMaps> NameMapsAsync(IEnumerable<Product> products)
    {
        var items = products.ToList();
        var catIds = items.Select(p => p.CategoryId).Distinct().ToList();
        var brandIds = items.Where(p => p.BrandId.HasValue).Select(p => p.BrandId!.Value).Distinct().ToList();
        var modelIds = items.Where(p => p.VehicleModelId.HasValue).Select(p => p.VehicleModelId!.Value).Distinct().ToList();
        var cats = await _db.Categories.AsNoTracking().Where(c => catIds.Contains(c.Id)).ToDictionaryAsync(c => c.Id, c => c.Name);
        var brands = await _db.Brands.AsNoTracking().Where(b => brandIds.Contains(b.Id)).ToDictionaryAsync(b => b.Id, b => b.Name);
        var models = await _db.VehicleModels.AsNoTracking().Where(m => modelIds.Contains(m.Id)).ToDictionaryAsync(m => m.Id, m => m.Name);
        return new NameMaps(cats, brands, models);
    }

    private async Task<Dictionary<int, (double Avg, int Count)>> ReviewAggAsync(IEnumerable<int> productIds)
    {
        var ids = productIds.Distinct().ToList();
        if (ids.Count == 0) return new();
        var rows = await _db.Reviews.AsNoTracking()
            .Where(r => ids.Contains(r.ProductId) && r.ReviewStatus == "Approved")
            .GroupBy(r => r.ProductId)
            .Select(g => new { ProductId = g.Key, Avg = g.Average(x => (double)x.Rating), Count = g.Count() })
            .ToListAsync();
        return rows.ToDictionary(x => x.ProductId, x => (x.Avg, x.Count));
    }

    private sealed record NameMaps(
        IReadOnlyDictionary<int, string> Categories,
        IReadOnlyDictionary<int, string> Brands,
        IReadOnlyDictionary<int, string> Models);

    private static ProductVariantDto MapVariant(Sku s, IReadOnlyDictionary<int, int> skuStock) => new(
        s.Id, s.SkuCode, s.VariantName, s.SalePrice ?? s.ListPrice, skuStock.GetValueOrDefault(s.Id),
        s.Color, s.Version, s.Status);

    private static ProductListItem MapListItem(
        Product p, string? manufacturerName, int stockTotal,
        NameMaps names, (double Avg, int Count) reviews, IReadOnlyDictionary<int, int> skuStock)
    {
        var sku = p.Skus.OrderBy(s => s.SalePrice ?? s.ListPrice).FirstOrDefault();
        var image = p.Images.OrderByDescending(i => i.IsPrimary).ThenBy(i => i.SortOrder).FirstOrDefault();
        var listPrice = sku?.ListPrice ?? 0;
        var salePrice = sku?.SalePrice;
        return new ProductListItem(
            p.Id, p.Code, p.Name, p.Slug, p.CategoryId, p.BrandId, p.VehicleModelId, p.Kind,
            p.IsFeatured, p.IsHotDeal, listPrice, salePrice, image?.Url, p.ManufacturerId, manufacturerName, stockTotal, p.Status,
            // ===== Lớp tương thích frontend cũ =====
            p.Code, names.Categories.GetValueOrDefault(p.CategoryId),
            p.BrandId.HasValue ? names.Brands.GetValueOrDefault(p.BrandId.Value) : null,
            p.VehicleModelId, p.VehicleModelId.HasValue ? names.Models.GetValueOrDefault(p.VehicleModelId.Value) : null,
            p.Kind, listPrice, stockTotal, reviews.Avg, reviews.Count,
            p.Skus.Select(s => MapVariant(s, skuStock)),
    ...
    private static ProductDetail MapDetail(
        Product p, string? manufacturerName,
        NameMaps names, (double Avg, int Count) reviews, IReadOnlyDictionary<int, int> skuStock)
    {
        var sku = p.Skus.OrderBy(s => s.SalePrice ?? s.ListPrice).FirstOrDefault();
        var image = p.Images.OrderByDescending(i => i.IsPrimary).ThenBy(i => i.SortOrder).FirstOrDefault();
        ...
        return new ProductDetail(
            ...
            p.BrandId.HasValue ? names.Brands.GetValueOrDefault(p.BrandId.Value) : null,
            p.VehicleModelId, p.VehicleModelId.HasValue ? names.Models.GetValueOrDefault(p.VehicleModelId.Value) : null,
            p.Kind, listPrice, salePrice, p.Skus.Sum(s => skuStock.GetValueOrDefault(s.Id)),
            image?.Url, reviews.Avg, reviews.Count,
            p.Skus.Select(s => MapVariant(s, skuStock)));
    }

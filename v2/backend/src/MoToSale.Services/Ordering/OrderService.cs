...
public class OrderService : IOrderService
{
    ...
    private readonly IRepository<RefundRequest> _refunds;
    ...
    public OrderService(
        ...
        IRepository<Setting> settings, IRepository<RefundRequest> refunds)
    {
        _cart = cart;
    ...
    public async Task<CartDto> ClearCartAsync(int userId)
    {
        ...
        {
            _cart.ClearItems(cart.Items.ToList());
            await _cart.SaveChangesAsync();
        }
        return await GetCartAsync(userId);
    }
    ...
    private const decimal FlatShippingFee = 30000m;   // Phí giao hàng cố định khi không nhận tại cửa hàng.
    private const string CfgBankName = "BankName";
    private const string CfgBankAccountNo = "BankAccountNo";
    private const string CfgBankAccountName = "BankAccountName";
    private const string CfgBankBin = "BankBin";       // Mã ngân hàng để dựng VietQR (tùy chọn).

    private static readonly HashSet<string> RefundableStatuses = new(StringComparer.OrdinalIgnoreCase)
    {
        OrderStatus.Confirmed, OrderStatus.Allocated, OrderStatus.Delivered, OrderStatus.Completed,
    };
    private static readonly HashSet<string> PaidStatuses = new(StringComparer.OrdinalIgnoreCase)
    {
        PaymentStatus.DepositPaid, PaymentStatus.PartiallyPaid, PaymentStatus.Paid,
    };

    private async Task<string?> GetSettingAsync(string key)
    {
        var found = await _settings.FindAsync(s => s.Key == key);
        return found.FirstOrDefault()?.Value;
    }

    public async Task<PaymentInfoDto> GetPaymentInfoAsync(int orderId)
    {
        var order = await _orders.GetByIdAsync(orderId) ?? throw new OrderException("Không tìm thấy đơn hàng.");

        // Số tiền còn phải trả: tính lại từ tổng đã thu để chính xác thời điểm xem.
        var totalPaid = await _payments.GetTotalPaidAsync(order.Id);
        var amountDue = order.OrderStatus == OrderStatus.Cancelled ? 0 : Math.Max(0, order.GrandTotal - totalPaid);

        // Lấy thông tin ngân hàng từ Setting, fallback giá trị dev nếu chưa cấu hình.
        var bankName = await GetSettingAsync(CfgBankName) ?? "Vietcombank";
        var accountNumber = await GetSettingAsync(CfgBankAccountNo) ?? "0123456789";
        var accountHolder = await GetSettingAsync(CfgBankAccountName) ?? "CONG TY MOTOSALE";
        var bin = await GetSettingAsync(CfgBankBin);

        var transferContent = order.Code;
        string? qrImageUrl = !string.IsNullOrWhiteSpace(bin) && amountDue > 0
            ? BuildVietQrUrl(bin!, accountNumber, accountHolder, amountDue, transferContent)
            : null;

        return new PaymentInfoDto(order.Code, amountDue, bankName, accountNumber, accountHolder, transferContent, qrImageUrl);
    }

    public ShippingQuoteResponse GetShippingQuote(ShippingQuoteRequest request)
    {
        // Nhận tại cửa hàng → miễn phí; còn lại tính phí giao hàng cố định.
        var method = request.ReceivingMethod?.Trim() ?? string.Empty;
        var isPickup = method.Contains("Store", StringComparison.OrdinalIgnoreCase)
            || method.Contains("Pickup", StringComparison.OrdinalIgnoreCase);
        return new ShippingQuoteResponse(isPickup ? 0m : FlatShippingFee);
    }

    public async Task<int> RequestRefundAsync(int orderId, int userId, RequestRefundRequest request)
    {
        var order = await _orders.GetByIdAsync(orderId) ?? throw new OrderException("Không tìm thấy đơn hàng.");
        if (order.UserId != userId) throw new OrderException("Bạn không có quyền yêu cầu hoàn tiền cho đơn hàng này.");
        if (order.OrderStatus == OrderStatus.Cancelled) throw new OrderException("Đơn đã hủy.");
        if (!PaidStatuses.Contains(order.PaymentStatus))
            throw new OrderException("Đơn hàng chưa có giao dịch thanh toán thành công để hoàn tiền.");
        if (!RefundableStatuses.Contains(order.OrderStatus))
            throw new OrderException("Đơn hàng không ở trạng thái có thể yêu cầu hoàn tiền.");

        var existing = await _refunds.FindAsync(r => r.OrderId == orderId && r.RefundStatus == RefundRequestStatus.Pending);
        if (existing.Count > 0) throw new OrderException("Đơn hàng đã có yêu cầu hoàn tiền đang chờ xử lý.");

        var now = DateTime.UtcNow;
        var refundAmount = await _payments.GetTotalPaidAsync(orderId);
        var refund = new RefundRequest
        {
            ...
            AccountHolder = request.AccountHolder?.Trim().ToUpperInvariant() ?? string.Empty,

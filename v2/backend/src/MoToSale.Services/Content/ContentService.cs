...
public class ContentService : IContentService
{
    ...
    // ===== Bài viết công khai =====
    public async Task<PagingResponse<PublicPostListItem>> GetPublishedPostsAsync(PagingRequest r)
    {
        var all = await _posts.GetAllAsync();
        var q = all.Where(IsPublished);
        if (!string.IsNullOrWhiteSpace(r.Keyword))
            q = q.Where(p => p.Title.Contains(r.Keyword!, StringComparison.OrdinalIgnoreCase));
        // Sắp xếp: bài mới xuất bản trước, fallback theo ngày tạo
        var ordered = q.OrderByDescending(p => p.PublishedAt ?? p.CreatedDate).ThenByDescending(p => p.Id).ToList();
        var items = ordered.Skip((r.Page - 1) * r.PageSize).Take(r.PageSize)
            .Select(p => new PublicPostListItem(p.Id, p.Title, p.Slug, p.Summary, p.CoverUrl, p.Category, p.PublishedAt, p.CreatedDate)).ToList();
        return new PagingResponse<PublicPostListItem> { Items = items, Page = r.Page, PageSize = r.PageSize, TotalItems = ordered.Count };
    }

    public async Task<PublicPostDto?> GetPublishedPostBySlugAsync(string slug)
    {
        if (string.IsNullOrWhiteSpace(slug)) return null;
        var all = await _posts.GetAllAsync();
        var p = all.FirstOrDefault(x => IsPublished(x) && string.Equals(x.Slug, slug.Trim(), StringComparison.OrdinalIgnoreCase));
        return p is null ? null : new PublicPostDto(p.Id, p.Title, p.Slug, p.Summary, p.Body, p.CoverUrl, p.Category, p.PublishedAt, p.CreatedDate);
    }

    // Bài viết được coi là công khai khi đã xuất bản và đang ở trạng thái Active
    private static bool IsPublished(Post p) =>
        p.Status == (int)EntityStatus.Active &&
        (string.Equals(p.PostStatus, "Published", StringComparison.OrdinalIgnoreCase) ||
         string.Equals(p.PostStatus, "Active", StringComparison.OrdinalIgnoreCase));
    ...
    public async Task<int> CreateContactRequestAsync(ContactRequestForm r)
    {
        if (string.IsNullOrWhiteSpace(r.HoTen)) throw new ContentException("Họ tên là bắt buộc.");
        if (string.IsNullOrWhiteSpace(r.SoDienThoai)) throw new ContentException("Số điện thoại là bắt buộc.");
        if (string.IsNullOrWhiteSpace(r.NoiDung)) throw new ContentException("Nội dung là bắt buộc.");
        var c = new ContactRequest
        {
            FullName = r.HoTen.Trim(),
            Phone = r.SoDienThoai.Trim(),
            Email = string.IsNullOrWhiteSpace(r.Email) ? null : r.Email.Trim(),
            Subject = string.IsNullOrWhiteSpace(r.TieuDe) ? null : r.TieuDe.Trim(),
            Body = r.NoiDung.Trim(),
            Type = string.IsNullOrWhiteSpace(r.LoaiYeuCau) ? "General" : r.LoaiYeuCau.Trim(),

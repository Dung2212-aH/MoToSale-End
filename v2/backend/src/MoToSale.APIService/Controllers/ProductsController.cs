...
public class ProductsController : ControllerBase
{
    ...
    public ProductsController(ICatalogService catalog, IImageStorage storage, IReviewService reviews)
    {
        _catalog = catalog;
        _storage = storage;
        _reviews = reviews;
    ...
    // ===== Tùy chọn lọc (công khai) =====
    [HttpGet("filters")]
    public async Task<IActionResult> GetFilters()
    {
        var categories = await _catalog.GetCategoriesAsync();
        var brands = await _catalog.GetBrandsAsync();
        var models = await _catalog.GetVehicleModelsAsync(null);
        return Ok(new ProductFiltersDto(categories, brands, models));
    }

    // ===== Đánh giá sản phẩm (khách hàng) =====
    [HttpGet("{productId:int}/reviews")]
    public async Task<IActionResult> GetReviews(int productId) =>
        Ok(new { items = await _reviews.GetApprovedAsync(productId) });

    [HttpGet("{productId:int}/reviews/summary")]
    public async Task<IActionResult> GetReviewSummary(int productId) =>
        Ok(await _reviews.GetSummaryAsync(productId));

    [Authorize]
    [HttpPost("{productId:int}/reviews")]
    [Consumes("multipart/form-data")]
    public async Task<IActionResult> CreateReview(int productId, [FromForm] CreateReviewForm form)
    {
        try
        {
            var file = form.FileValue;
            string? imageUrl = null;
            if (file is { Length: > 0 })
                imageUrl = await _storage.SaveAsync(file, "reviews", HttpContext.RequestAborted);

            var review = await _reviews.CreateAsync(productId, CurrentUserId, form.RatingValue, form.TitleValue, form.CommentValue, imageUrl);
            return Ok(new { message = "Đánh giá của bạn đã được gửi và đang chờ duyệt.", review });
        }
        catch (InvalidOperationException ex) { return BadRequest(new { message = ex.Message }); }
        catch (CatalogException ex) { return BadRequest(new { message = ex.Message }); }
    }
    ...
    [Authorize]
    [HttpPatch("{productId:int}/reviews/me")]
    [Consumes("multipart/form-data")]
    public async Task<IActionResult> UpdateMyReview(int productId, [FromForm] UpdateMyReviewForm form)
    {
        try
        {
            var file = form.FileValue;
            ...
            var review = await _reviews.UpdateMineAsync(productId, CurrentUserId, form.RatingValue, form.TitleValue, form.CommentValue, imageUrl);
            return Ok(new { message = "Đánh giá của bạn đã được cập nhật và đang chờ duyệt lại.", review });
        }
        catch (InvalidOperationException ex) { return BadRequest(new { message = ex.Message }); }
        catch (ReviewNotFoundException ex) { return NotFound(new { message = ex.Message }); }
        catch (CatalogException ex) { return BadRequest(new { message = ex.Message }); }
    }
}

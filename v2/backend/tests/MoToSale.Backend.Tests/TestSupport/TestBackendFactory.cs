...
public sealed class TestBackendFactory
{
    ...
    public CatalogService CatalogService() => new(
        ...
        new Repository<Favorite>(Db),
        new InventoryRepository(Db),
        Db);

    public VoucherService VoucherService() => new(new VoucherRepository(Db), new Repository<UserVoucher>(Db));
    ...
    public OrderService OrderService() => new(
        ...
        new Repository<MoToSale.Entities.SystemConfig.Setting>(Db),
        new Repository<RefundRequest>(Db));
}

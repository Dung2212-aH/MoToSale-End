...
// Gỡ bọc mảng EF ($values) / items / Items về một mảng phẳng.
const arrayOf = (value) => {
  if (Array.isArray(value)) return value;
  return value?.$values || value?.items || value?.Items || [];
};

// Chuẩn hóa kế hoạch trả góp: giữ nguyên tên field nghiệp vụ (tiếng Việt) nhưng luôn trả terms là mảng phẳng.
export const mapInstallmentPlan = (raw = {}) => {
  const plan = field(raw, 'traGop', 'TraGop');
  if (!plan) return null;
  return {
    ...plan,
    terms: arrayOf(field(plan, 'terms', 'Terms')),
  };
};
...
export const mapOrder = (raw = {}) => {
  ...
  return {
    ...raw,
    traGop: mapInstallmentPlan(raw),
    yeuCauHoanTien: arrayOf(field(raw, 'yeuCauHoanTien', 'YeuCauHoanTien')),

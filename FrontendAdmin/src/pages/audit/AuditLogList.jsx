              ...
              {loading ? (
                ...
                logs.map((log) => (
                  <tr key={log.maNhatKy || log.id}>
                    ...
                    <td>{formatLogTime(log.thoiGian || log.createdAt)}</td>
                    <td>{labelOf(entityLabels, log.loaiDoiTuong || log.entityType)}</td>
                    ...
                    <td>
                      ...
                      {(log.maNguoiThucHien || log.actorUserId) && (
                        <span className="text-muted ml-1">#{log.maNguoiThucHien || log.actorUserId}</span>
                      )}
                    </td>
                    <td className="text-break">{log.ghiChu || log.note || '-'}</td>
                  </tr>
                ))
              )}
            </tbody>

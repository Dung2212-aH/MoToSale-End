...
const BusinessOperations = () => {
  ...
  const load = async () => {
    try {
      const [lookup, stats, suppliers, purchases, cashRows, repairs, crmRows, attendanceRows] = await Promise.all([
        service.getLookups(),
      ...
      setData({
        ...
        attendance: attendanceRows.data.items || [],
      });
    } catch (err) {
  ...
  const openCreate = () => {
    ...
    if (tab === 'attendance') setAttendance({ ...emptyAttendance, staffUserId: isAdmin() ? '' : (user?.id || user?.Id || '') });
    setModal(tab === 'suppliers' ? 'supplier' : tab === 'purchases' ? 'purchase' : tab === 'cash' ? 'cash' : tab === 'repairs' ? 'repair' : tab === 'crm' ? 'crm' : 'attendance');
  };
  ...
  const importSuppliers = async (event) => {
    ...
    try {
      ...
      for (let index = 2; index <= sheet.rowCount; index += 1) {
        ...
        try { await service.createSupplier(payload); saved += 1; }
        catch (err) { errors.push(`Dòng ${index}: ${msg(err, 'không thể lưu')}`); }
      }
      await load();
  ...
  const printRecord = (title, rows) => {
    ...
    if (!popup) return;
    popup.document.write(`<html><head><title>${title}</title><style>body{font-family:Arial,sans-serif;padding:28px;color:#222}h1{font-size:22px}table{border-collapse:collapse;width:100%;margin-top:18px}th,td{border:1px solid #ddd;padding:9px;text-align:left}th{width:32%;background:#f3f4f6}.sign{display:grid;grid-template-columns:1fr 1fr;gap:80px;text-align:center;margin-top:54px}</style></head><body><h1>MoToSale - ${title}</h1><table>${rows.map(([label, value]) => `<tr><th>${label}</th><td>${value ?? '-'}</td></tr>`).join('')}</table><div class="sign"><div>Người lập phiếu<br/><br/><br/>........................</div><div>Người xác nhận<br/><br/><br/>........................</div></div><script>window.onload=()=>window.print()</script></body></html>`);
  ...
  return (
    <div className="content-wrapper">
      <div className="content-header"><div className="container-fluid"><h1 className="m-0">Nghiệp vụ cửa hàng</h1></div></div>
      <section className="content"><div className="container-fluid">
        <div className="row">
          {[
            ...
            ['Giá trị mua hàng (toàn kỳ)', formatCurrency(summary.purchaseValue)],
            ['Thu ròng (toàn kỳ)', formatCurrency((summary.cashIn || 0) - (summary.cashOut || 0))],
            ['Phiếu sửa đang mở', summary.openRepairs],
            ['CSKH cần xử lý', summary.openInteractions],
          ].filter(([label]) => isAdmin() || !['Giá trị mua hàng (toàn kỳ)', 'Thu ròng (toàn kỳ)'].includes(label)).map(([label, value]) => (
            <div className="col-lg-2 col-md-4 col-6" key={label}><div className="small-box bg-light"><div className="inner"><h4>{value || 0}</h4><p>{label}</p></div></div></div>
          ))}
        </div>

        <div className="card">
          <div className="card-header border-b border-slate-200 px-4 pt-3">
            <div className="flex flex-wrap gap-x-2 gap-y-1" role="tablist" aria-label="Nhóm nghiệp vụ cửa hàng">
              {tabs.filter(([key]) => isAdmin() || key !== 'cash').map(([key, label]) => (
          ...
          <div className="card-body">
            {(isAdmin() || !['suppliers', 'cash'].includes(tab)) && <button className="btn btn-primary mb-3" onClick={openCreate}><i className="fas fa-plus mr-1" />{tab === 'attendance' ? 'Check-in' : 'Tạo mới'}</button>}
            ...
            {tab === 'crm' && (
              <>
                <div className="row mb-3">
                  <div className="col-md-3"><select className="form-control" value={crmStatus} onChange={(e) => setCrmStatus(e.target.value)}><option value="">Tất cả trạng thái</option><option value="Open">Đang mở</option><option value="Completed">Đã hoàn thành</option></select></div>
                  <div className="col-md-5"><input className="form-control" value={crmKeyword} onChange={(e) => setCrmKeyword(e.target.value)} placeholder="Tìm khách hàng, nội dung, ghi chú..." /></div>
                </div>
                <CrmTable rows={visibleRows} run={run} edit={(x) => { setCrm(x); setModal('crm'); }} />
              </>
            )}
            {tab === 'attendance' && <AttendanceTable rows={visibleRows} run={run} />}
            {totalPages > 1 && <div className="d-flex justify-content-between align-items-center mt-3"><span>Trang {page}/{totalPages} · {filteredRows.length} bản ghi</span><div><button className="btn btn-outline-secondary btn-sm mr-2" disabled={page === 1} onClick={() => setPage((value) => value - 1)}>Trước</button><button className="btn btn-outline-secondary btn-sm" disabled={page === totalPages} onClick={() => setPage((value) => value + 1)}>Sau</button></div></div>}
          </div>
        </div>
      </div></section>

      {modal && (
        <Modal title={modalTitle} close={() => setModal('')} save={save}>
          ...
          {modal === 'attendance' && <AttendanceForm value={attendance} set={setAttendance} lookups={lookups} lockStaff={!isAdmin()} />}
        </Modal>
      )}
      ...
      {paymentTarget && <Modal title={`Thanh toán NCC - ${paymentTarget.code}`} close={() => setPaymentTarget(null)} save={payPurchase}><Input label="Số tiền" type="number" value={payment.amount} set={(x) => setPayment({ ...payment, amount: x })} /><Select label="Hình thức" value={payment.method} set={(x) => setPayment({ ...payment, method: x })} items={[{ id: 'Cash', name: 'Tiền mặt' }, { id: 'BankTransfer', name: 'Chuyển khoản' }]} text={(x) => x.name} /><Input label="Ghi chú" value={payment.note} set={(x) => setPayment({ ...payment, note: x })} /></Modal>}
...
const Input = ({ label, value, set, type = 'text' }) => <div className="form-group"><label>{label}</label><input className="form-control" type={type} value={value} onChange={(e) => set(e.target.value)} /></div>;

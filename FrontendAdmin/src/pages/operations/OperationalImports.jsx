  ...
  const commit = async () => {
    ...
    try {
      if (tab === 'products') {
        ...
        for (const row of rows) {
          try {
            const sku = (lookups.skus || []).find((x) => x.id === row.skuId);
            if (!sku) throw new Error('Mã SKU không tồn tại');
            await inventoryService.adjustStock({ maSanPham: sku.maSanPham, maBienSanPham: row.skuId, soLuong: row.qty, loaiGiaoDich: 'Adjust', lyDo: 'Import tồn đầu kỳ từ XLSX' });
